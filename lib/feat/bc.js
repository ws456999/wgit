...
export function gitBranchCheckout (exec, options = []) {
  // options = options
      ...
      .prompt([
        {
          ...
          choices: branches
            .filter(function filterOutDetachedMsg (branchName) {
              return !/\(detached\\ from\\ .*\)/.test(branchName)
            })
            .map(function formatBranchName (branchName) {
